...
def ja_agent(nama_posisi: str, band_posisi: str, retrieve_data: str, job_responsibilities: str, mission_statement: str):
    ...
    user_prompt = f"""
Buatkan Job Authorities untuk posisi berikut berdasarkan Mission Statement dan Job Responsibility: Nama Posisi: {nama_posisi} dengan band posisi nya : {band_posisi}. INGAT BAHWA NAMA FUNGSI HANYA DISEMATKAN PADA POIN KEDUA JA, JANGAN SEMATKAN NAMA FUNGSI DI POIN SATU DAN TIGA. Output langsung berupa list item dari JA, output tanpa kata pengantar, antar list item dipisahkan dengan penanda strip (-) BUKAN NOMOR, dan tidak dibold !
